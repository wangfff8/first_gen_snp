...
from . import logger as vsat_logger
...
def colorize_multiple_alignment_html(
    seqs_dict: dict[str, str], line_length: int = 80
) -> str:
    """
    ...
    html_output_lines.append(
        "pre { font-family: 'Courier New', Courier, monospace; white-space: pre; margin: 0; padding: 10px; }"
    )
    ...
    for i in range(alignment_length):
        ...
        for seq_idx in range(num_sequences):
            ...
            if char == "-":
                colored_sequences_chars[seq_idx].append(
                    f'<span class="gap">{char}</span>'
                )
            elif is_conserved and has_non_gap:
                colored_sequences_chars[seq_idx].append(
                    f'<span class="green">{char}</span>'
                )
            else:
                colored_sequences_chars[seq_idx].append(
                    f'<span class="red">{char}</span>'
                )
    ...
    for i in range(0, alignment_length, line_length):
        for seq_idx, header in enumerate(headers):
            ...
            html_output_lines.append(
                f'<span class="header">{display_header}</span>{segment_html}'
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        description="VSAT: Colorizes a multiple sequence alignment (FASTA) into an HTML file.",
    )
    ...
    display_group.add_argument(
        ...
        type=int,
        default=80,
        help="Number of characters per line in the alignment display.",
    )
    ...
    try:
        ...
        with open(args.output, "w", encoding="utf-8") as f:
            f.write(html_output)
        logger.info(f"Colored alignment saved to:\n - {args.output}")
