...
def align_and_pad_sequence(
    ref_seq: str, query_seq: str, n: int = 15, max_attempts: int = 20
) -> str:
    """
    Align a query sequence to a reference sequence using a robust seed search.

    This function attempts to find the starting position of the query sequence
    within the reference by searching for initial segments of length `n`.
    It tries multiple starting offsets within the query sequence to account
    for potential mutations or sequencing errors at the very beginning.

    Args:
        ref_seq: The reference DNA sequence.
        query_seq: The query DNA sequence to be aligned and padded.
        n: The length of the seed segment used for searching. Defaults to 15.
        max_attempts: The number of different starting offsets in the query
                      to try before giving up. Defaults to 20.

    Returns:
        The query sequence, padded with 'N's to match the reference length.

    Raises:
        ValueError: If no suitable alignment seed can be found in the reference.
    """
    start_pos_in_ref = -1
    offset_in_query = 0

    for i in range(max_attempts):
        seed = query_seq[i : i + n]
        if len(seed) < n:
            break

        found_pos = ref_seq.find(seed)
        if found_pos != -1:
            start_pos_in_ref = found_pos - i
            if start_pos_in_ref >= 0:
                break

    if start_pos_in_ref == -1:
        raise ValueError(
            f"Could not find an alignment seed of length {n} in the first "
            f"{max_attempts + n} bases of the query sequence."
        )

    padding_left = "N" * start_pos_in_ref
    padding_right = "N" * max(0, len(ref_seq) - (start_pos_in_ref + len(query_seq)))

    # Ensure the final padded sequence length matches the reference (truncate if needed)
    padded_seq = padding_left + query_seq + padding_right
    return padded_seq[: len(ref_seq)]
